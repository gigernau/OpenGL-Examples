	...
	GLfloat beige[]={0.96,0.96,0.86,1.0};
...
void mattone2(){
	...
	glMaterialfv(GL_FRONT_AND_BACK,GL_AMBIENT,light_B);
	glMaterialfv(GL_FRONT_AND_BACK,GL_SPECULAR,light_B);
	
	

	glutSolidCube(0.10);
	glMaterialfv(GL_FRONT_AND_BACK,GL_AMBIENT,light_N);
}
void mattone3(){
...
void drawScene( void )
	...
	glPushMatrix();
		...
		float i,j;
		glTranslatef(yRef,0.0,zRef);
		
		glPushMatrix();
			glLightfv(GL_LIGHT2, GL_AMBIENT, light_B);
			glLightfv(GL_LIGHT2, GL_POSITION, light_position);
			glPushMatrix();
					for(i=-3.0; i<3.0 ; i=i+0.10)
				{
					for (j = -10.0; j < 25.0; j=j+0.10)
					{	
						 glPushMatrix();
			...
			glTranslatef(3.0,0.0,0.0);
					for(i=-1.5; i<1.5 ; i=i+0.10)
				{
					for (j = -10.0; j < 25.0; j=j+0.10)
					{	
						 glPushMatrix();
						 glTranslatef(0.0,i,j);
						mattone();
						//quadratino2(i,j);
						glPopMatrix();
